...
BEGIN
    ...
    BEGIN
        ...
        BEGIN CATCH 
            ...
            BEGIN 
                IF @ErrorNumber BETWEEN 50001 AND 50011
                    ;THROW  -- Rethrow original error with its message
                ELSE
                    SET @ErrorMsg = 'Failed to admit patient after 3 retries.'
                    ;THROW 50012, @ErrorMsg, 1;
            END

            -- WAITFOR DELAY '00:00:01';  -- apparently this is haram
        END CATCH
    END
...
BEGIN
    BEGIN TRY
        INSERT INTO PatientTBL WITH (ROWLOCK)
            (PatientFname, PatientLname, PatientWard, PatientDependency, PatientCOVIDStatus)
